...
@section('content')
    <div class="row justify-content-center">
        <h1>Sortie :{{ $boutique->nom }} vers {{ $magasin->nom }}</h1>
        <div class="mb-3">
            <a href="{{ url('admin/boutique/' . $boutique->nom . '/sortie') }}" class="btn btn-dark">
                Retour
            </a>
            @if (Auth::user()->role_as == '1')
                <a href="{{ url('admin/boutique/'.$boutique->nom.'/sortie/'.$magasin->nom.'/create') }}"
                    class="btn btn-dark">
                    Ajout une operation
                </a>
            @endif
            <a href="{{ url('admin/boutique/'.$boutique->nom.'/sortie/'.$magasin->nom.'/historiques') }}"
                class="btn btn-dark">
                Historiques
            </a>
        </div>
                        ...
                        <tbody>
                            ...
                            @forelse ($operations as $operation)
                                <tr>
                                    ...
                                    {{-- @if (Auth::user()->role_as == '1')
                                        <td>
                                            <a href="{{ url('admin/boutique/' . $boutique->nom . '/sortie/' . $magasin->nom .'/edit/'.$operation->id) }}" class="btn btn-dark">Modifier</a>
                                        </td>
                                    @endif --}}
                                </tr>
